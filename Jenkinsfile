  ...
  stages {
    ...
    stage('Check Semaphore') {
      when {
        anyOf {
          branch 'master'
          expression { return params.CHECK_SEMAPHORE }
        }
      }
      steps {
        sh "bnw_runner ./_pipeline/step_check_semaphore.sh"
      }
      post {
        failure {
          script {
            sh "bnw_runner ./_pipeline/utils/slack_notification_on_master.sh ${PROJECT} FAILURE '${PROJECT} SEMAPHORE CHECK has failed'"
          }
        }
      }
    }
